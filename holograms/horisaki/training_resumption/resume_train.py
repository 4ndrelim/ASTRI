...
MODEL_DIR = os.path.join(BASE_DIR, 'new_saved_model')
...
TEST_PATH = os.path.join(LOADED_DATA_DIR, "test.npy")
NEW_MODEL_PATH = os.path.join(MODEL_DIR, "my_model.pth") # can be None -> will not save
SCALER_PATH = LOADED_SCALER_PATH  # should be the same as trained before
...
def feature_scaling(train: np.ndarray,
                    test: np.ndarray,
                    scaler: BaseEstimator) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def prepare_data_for_training(train_path: str, test_path: str, dev: torch.device,
                              scaler: BaseEstimator) -> Tuple[DataLoader, DataLoader, DataLoader]:
    """
    ...
    # perform scaling
    X_train_scaled, X_test_scaled = feature_scaling(X_train, X_test, scaler)
...
if __name__ == "__main__":
    ...
    my_model = MultiscaleResNet(1, 1,
                                ...
                                criterion=LOSS_FN)
    my_model.load_state_dict(torch.load(LOADED_MODEL_PATH,
                                     map_location=device))
