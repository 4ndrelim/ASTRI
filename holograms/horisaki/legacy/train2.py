...
def evaluate_model_reconstructed(model: MultiscaleResNet,
    ...
    with torch.no_grad():
        for X, y in data_loader:
            ...
            normalized_unscaled_X, normalized_z_np = normalize(unscaled_X), normalize(z_np)

            for i in range(unscaled_X.shape[0]):
                total_mae += mean_absolute_error(np.ravel(normalized_unscaled_X[i][0]), 
                                                 np.ravel(normalized_z_np[i][0]))
...
if __name__ == "__main__":
    ...
    test_mae1 = evaluate_model_reconstructed(my_model, test_set)
    print(f"MAE on TEST set: {test_mae1}")

    np.save(os.path.join(BASE_DIR, "train2_results.npy"), TRACKED_NP)
